...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, SINGLE_BUTTON, PROTECT_CONTENT, \
    SPELL_CHECK_REPLY, IMDB_TEMPLATE, IMDB_DELET_TIME, NO_RESULTS_MSG, START_MESSAGE, PMFILTER, G_FILTER, BUTTON_LOCK, SUPPORT_CHAT_ID, BUTTON_LOCK_TEXT, NOR_IMG, SPELL_IMG
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('<𝖻>👋𝖧𝖾𝗒 {message.from_user.mention}\n\n𝖲𝗈𝗋𝗋𝗒, 𝖭𝗈 𝖬𝗈𝗏𝗂𝖾/𝖲𝖾𝗋𝗂𝖾𝗌 𝖱𝖾𝗅𝖺𝗍𝖾𝖽 𝗍𝗈 𝗍𝗁𝖾 𝖦𝗂𝗏𝖾𝗇 𝖶𝗈𝗋𝖽 𝖶𝖺𝗌 𝖥𝗈𝗎𝗇𝖽😔\n\n𝖯𝗅𝖾𝖺𝗌𝖾 𝖦𝗈 𝗍𝗈 𝖦𝗈𝗈𝗅𝗀𝖾 𝖺𝗇𝖽 𝖢𝗈𝗇𝖿𝗂𝗋𝗆 𝗍𝗁𝖾 𝖢𝗈𝗋𝗋𝖾𝖼𝗍 𝖲𝗉𝖾𝗅𝗅𝗂𝗇𝗀🙏\n\n 𝖢𝗅𝗂𝖼𝗄 𝖧𝖾𝗋𝖾 𝖳𝗈 👉 <a href="https://www.google.com">🔍 𝖲𝖾𝖺𝗋𝖼𝗁 🔎</a>\n\n✍𝖮𝗋 𝖸𝗈𝗎𝗋 𝖲𝗉𝖾𝗅𝗅𝗂𝗇𝗀 𝖨𝗌 𝖢𝗈𝗋𝗋𝖾𝖼𝗍 𝖱𝖾𝗉𝗈𝗋𝗍 𝖳𝗈 𝖠𝖽𝗆𝗂𝗇𝗌 𝖥𝗈𝗋 𝖠𝖽𝖽 𝖱𝖾𝗊𝗎𝖾𝗌𝗍𝖾𝖽 𝖥𝗂𝗅𝖾 :- @𝖺𝖽𝗆𝗂𝗇𝗌</b>')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):        
        ...
        buttons = [[
            InlineKeyboardButton('« 𝐁𝐚𝐜𝐤 »', callback_data='help3')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        if settings is not None:
            buttons = [
                [
                    InlineKeyboardButton('▪️𝐅𝐢𝐥𝐭𝐞𝐫 𝐁𝐮𝐭𝐭𝐨𝐧',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}'),
                    InlineKeyboardButton('𝐒𝐢𝐧𝐠𝐥𝐞' if settings["button"] else '𝐃𝐨𝐮𝐛𝐥𝐞',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('🤖𝐁𝐨𝐭 𝐏𝐦', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}'),
                    InlineKeyboardButton('√ 𝐘𝐞𝐬' if settings["botpm"] else '⛌ 𝐍𝐨',
                                         callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('🗂𝐅𝐢𝐥𝐞 𝐒𝐞𝐜𝐮𝐫𝐞',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}'),
                    InlineKeyboardButton('√ 𝐘𝐞𝐬' if settings["file_secure"] else '⛌ 𝐍𝐨',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('🌄𝐈𝐦𝐝𝐛', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}'),
                    InlineKeyboardButton('√ 𝐘𝐞𝐬' if settings["imdb"] else '⛌ 𝐍𝐨',
                                         callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('🔎𝐒𝐩𝐞𝐥𝐥 𝐂𝐡𝐞𝐜𝐤',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
                    InlineKeyboardButton('√ 𝐘𝐞𝐬' if settings["spell_check"] else '⛌ 𝐍𝐨',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('🫂𝐖𝐞𝐥𝐜𝐨𝐦𝐞 𝐌𝐠', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}'),
                    InlineKeyboardButton('√ 𝐘𝐞𝐬' if settings["welcome"] else '⛌ 𝐍𝐨',
...
async def pm_spoll_choker(msg):
    ...
    k = await msg.reply_photo(photo=SPELL_IMG, caption="<b>𝖨 𝖼𝗈𝗎𝗅𝖽𝗇'𝗍 𝖿𝗂𝗇𝖽 𝖺𝗇𝗒𝗍𝗁𝗂𝗇𝗀 𝗋𝖾𝗅𝖺𝗍𝖾𝖽 𝗍𝗈 𝗍𝗁𝖺𝗍 𝖽𝗂𝖽 𝗒𝗈𝗎 𝗆𝖾𝖺𝗇 𝖺𝗇𝗒 𝗈𝗇𝖾 𝗈𝖿 𝗍𝗁𝖾𝗋𝖾..!</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=msg.id)
